-- noqa: disable=all
{% macro bulk_load_int_card_view_web_monthly(p_activity_year, p_activity_month) %}

{#
-- dbt run-operation bulk_load_int_card_view_web_monthly --args '{p_activity_year: 2019, p_activity_month: 1}'
    ...
    {% if execute %}
        ...
        #}
            ...
            {% if is_valid_date == True %}
                ...
                merge into ANALYTICS.central_staging.int_job__card_views_web_nwt as DBT_INTERNAL_DEST
                using
                (
                    with
card_view_nwt as (
    select *
    from analytics.central_staging.int_job__cardview_web
    where
    true
...
adobe_datafeeds as (
    select *
    from analytics.central_staging.int_job__adobe_datafeeds_web
),
