-- noqa: disable=all
{% macro bulk_load_int_jobs__job_posting_card_view_native_monthly(p_activity_year, p_activity_month) %}

{#
-- dbt run-operation bulk_load_int_jobs__job_posting_card_view_native_monthly --args '{p_activity_year: 2024, p_activity_month: 1}'
-- This macro loads bulk data into int_jobs__job_posting_card_view_native for a given year and month
    ...
    {% if execute %}
        ...
        #}
            ...
            {% if is_valid_date == True %}
                ...
                merge into INTERMEDIATE.JOBS.job_posting_card_view_native as DBT_INTERNAL_DEST
                using
                (
                    with
card_view_nwt as (
    select *
    from intermediate.jobs.job_posting_card_view_search_native
    where
    true
...
adobe_datafeeds as (
    select *
    from intermediate.jobs.job_posting_card_view_adobe_native
)
...
from
    ...
    left join adobe_datafeeds as df
            on
            (
                (nwt.xing_user_id = df.xing_user_id)
                and (
